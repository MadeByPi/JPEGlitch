package madebypi.labs.glitch.jpeg {
	 ...
	 *
	 * A modified version of the optimised JPEGEncoder avilable at bytearray.org
	 * http://www.bytearray.org/?p=775
	 *
	 * Added a few tweakable properties to 'glitch things up a bit'
	 * and changed a few vars to const values)
	 */
	public final class JPEGlitchEncoder {
		...
		private function fDCTQuant(data:Vector.<Number>, fdtbl:Vector.<Number>):Vector.<int> {
			...
			for (i=0; i<I8; ++i) {
				...
				const tmp0:Number = d0 + d7;
				const tmp7:Number = d0 - d7;
				const tmp1:Number = d1 + d6;
				const tmp6:Number = d1 - d6;
				const tmp2:Number = d2 + d5;
				const tmp5:Number = d2 - d5;
				const tmp3:Number = d3 + d4;
				const tmp4:Number = d3 - d4;
				
				/* Even part */
				var tmp10:Number = tmp0 + tmp3;	/* phase 2 */
				const tmp13:Number = tmp0 - tmp3;
				...
				const z1:Number = (tmp12 + tmp13) * 0.707106781; /* c4 */
				...
				const z5:Number = (tmp10 - tmp12) * 0.382683433; /* c6 */
				const z2:Number = 0.541196100 * tmp10 + z5; /* c2-c6 */
				const z4:Number = 1.306562965 * tmp12 + z5; /* c2+c6 */
				const z3:Number = tmp11 * 0.707106781; /* c4 */
				
				const z11:Number = tmp7 + z3;	/* phase 5 */
				const z13:Number = tmp7 - z3;
			...
			for (i=0; i<I8; ++i) {
				...
				const tmp0p2:Number = d0 + d7;
				const tmp7p2:Number = d0 - d7;
				const tmp1p2:Number = d1 + d6;
				const tmp6p2:Number = d1 - d6;
				const tmp2p2:Number = d2 + d5;
				const tmp5p2:Number = d2 - d5;
				const tmp3p2:Number = d3 + d4;
				const tmp4p2:Number = d3 - d4;
	
				/* Even part */
				var tmp10p2:Number = tmp0p2 + tmp3p2;	/* phase 2 */
				const tmp13p2:Number = tmp0p2 - tmp3p2;
				...
				const z1p2:Number = (tmp12p2 + tmp13p2) * 0.707106781; /* c4 */
				...
				/* The rotator is modified from fig 4-8 to avoid extra negations. */
				const z5p2:Number = (tmp10p2 - tmp12p2) * 0.382683433; /* c6 */
				const z2p2:Number = 0.541196100 * tmp10p2 + z5p2; /* c2-c6 */
				const z4p2:Number = 1.306562965 * tmp12p2 + z5p2; /* c2+c6 */
				const z3p2:Number= tmp11p2 * 0.707106781; /* c4 */
				
				const z11p2:Number = tmp7p2 + z3p2;	/* phase 5 */
				const z13p2:Number = tmp7p2 - z3p2;
			...
			for (i=0; i<I64; ++i){
				...
				outputfDCTQuant[i] = (fDCTQuant > 0.0) ? int(fDCTQuant + 0.5) : int(fDCTQuant - 0.5);
			}
		...
		/**
		 * Modified the write DQT a little
		 * Creating a seperate haeader block for each table rather than having Y+UV inline in one block
		 */
		private function writeDQT():void {
			...
			byteout.writeShort(67);	// length
			byteout.writeByte(0); // id
			for (i=0; i<I64; ++i) byteout.writeByte(YTable[i]); // write the Y table
			
			byteout.writeShort(0xFFDB); // marker
			byteout.writeShort(67);	// length
			byteout.writeByte(1); // id
			for (i=0; i<I64; ++i) byteout.writeByte(UVTable[i]); // write the UV table
		}
		
		/**
		 * Modified the writing of DHT a little...
		 * Creating a seperate header block for each component rather than having them all inline in one block (much easier to parse the dht this way)
		 * This may affect the encode performance very slightly... a couple more writeBytes.. But not by much.
		 */
		private function writeDHT():void {
			...
			// DHT Class=0 ID=0 - DC component of Luminance (Y)
			byteout.writeShort(0xFFC4); //marker
			byteout.writeShort(31); //length
			byteout.writeByte(0); // HTYDCinfo
			for (i=0; i<I16; ++i) byteout.writeByte(std_dc_luminance_nrcodes[int(i+1)]);
			for (i=0; i<=I11; ++i) byteout.writeByte(std_dc_luminance_values[i]);
			
			// DHT Class=1 ID=0 - AC component of Luminance (Y)
			byteout.writeShort(0xFFC4); //marker
			byteout.writeShort(181); //length
			byteout.writeByte(0x10); // HTYACinfo
			for (i=0; i<I16; ++i) byteout.writeByte(std_ac_luminance_nrcodes[int(i+1)]);
			for (i=0; i<=I161; ++i) byteout.writeByte(std_ac_luminance_values[i]);
			
			// DHT Class=0 ID=1 - DC component of Chrominance (Cb & Cr)
			byteout.writeShort(0xFFC4); //marker
			byteout.writeShort(31); //length
			byteout.writeByte(1); // HTUDCinfo
			for (i=0; i<I16; ++i) byteout.writeByte(std_dc_chrominance_nrcodes[int(i+1)]);
			for (i=0; i<=I11; ++i) byteout.writeByte(std_dc_chrominance_values[i]);
			
			// DHT Class=1 ID=1 - AC component of Chrominance (Cb & Cr)
			byteout.writeShort(0xFFC4); //marker
			byteout.writeShort(181); //length
			byteout.writeByte(0x11); // HTUACinfo
			for (i=0; i<I16; ++i) byteout.writeByte(std_ac_chrominance_nrcodes[int(i+1)]);
			for (i=0; i<=I161; ++i) byteout.writeByte(std_ac_chrominance_values[i]);
		}
		
		private function writeSOS():void {
			byteout.writeShort(0xFFDA); // marker
			byteout.writeShort(12); // length
			byteout.writeByte(3); // nrofcomponents - Y Cb Cr
			byteout.writeByte(1); // IdY
		...
		private function processDU(CDU:Vector.<Number>, fdtbl:Vector.<Number>, DC:Number, HTDC:Vector.<BitString>, HTAC:Vector.<BitString>):Number {
			...
			const I63:int = 63;
			const I64:int = 64;
			const DU_DCT:Vector.<int> = fDCTQuant(CDU, fdtbl);
			//ZigZag reorder
			for (var j:int=0;j<I64;++j) DU[ZigZag[j]]=DU_DCT[j];
			
			const Diff:int = DU[0] - DC;
			DC = DU[0];
		...
		public var glitchA		:Number	= 0.0;
		public var glitchAMod	:Number = 0.1;
		public var glitchB		:Number =  0.0;
		public var glitchC		:Boolean = false;
		public var glitchD		:Boolean = false;
		public var xOffAmt		:Number = 0;
		public var yOffAmt		:Number = 0;
		public var _quality		:uint   = uint.MIN_VALUE;
		...
		public function JPEGlitchEncoder(quality:int = 50) {
			init();
		...
		private function init():void {
			...
			// moved initQuantTables to set:quality for changing the quality setting without recreating the whole class.. ;)
			// initQuantTables(sf);
		}
		
		/**
		 * Encode an image to JPEG bytes
		 * @param	image		BitmapData to encode
		 * @param	quality		Quality to encode at (1-100)
		 * 						Encoder will use the current quality setting if omitted (-1)
		 * @return	JPEG ByteArray data
		 */
		public function encode(image:BitmapData, quality:int=-1):ByteArray {
			...
			const width :int = image.width;
			const height:int = image.height;
			const xo	:Number = 2 + xOffAmt;
			const yo	:Number = 2 + yOffAmt;
			
			for (var ypos:int=0; ypos<height; ypos+=8) {
				for (var xpos:int=0; xpos<width; xpos+=8) {
					if(glitchD){ // DU processing reversed
						RGB2YUV(image, xpos + int((Math.random() - 0.5) * xo), ypos + int((Math.random() - 0.5) * yo));
					...
					//
					if (glitchA > 0.0) DCU += glitchA * ((DCV * (Math.random() - 0.5)) & glitchAMod * (DCY * -Math.random()));
					//
					if (glitchB > 0.0) DCY ^= uint(Math.random() * (1 + glitchB));
				}
			}
			
			// Do the bit alignment of the EOI marker
			if ( bytepos >= 0 ){
				const fillbits:BitString = new BitString();
		...
		public function get bytes():ByteArray { return byteout; }
		
		public function set quality(value:uint):void {
			if (value < 1)   value  = 1;
			if (value > 100) value = 100;
			if (_quality != value) { // only update if it's different
				_quality 	= value;
				sf 			= (value < 50) ? int(5000 / value) : int(200 - (value << 1));
				initQuantTables(sf);
			}
		}
...
final internal class BitString {
